...
// Tipagem para evitar o erro de tipo implícito 'any' nas relações do Supabase
            ...
            <Card className="p-0 overflow-hidden">
              {loading ? (
                ...
                <Table>
                  ...
                  <TableBody>
                    {payments.map((p) => {
                      // Tratamento de segurança para dados relacionais vindos do Supabase (objetos ou arrays)
                      const studentName = p.student
                        ? (Array.isArray(p.student) ? p.student[0]?.full_name : p.student.full_name)
                        : "Desconhecido";

                      const courseName = p.course
                        ? (Array.isArray(p.course) ? p.course[0]?.name : p.course.name)
                        : "Desconhecida";

                      return (
                        <TableRow key={p.id}>
                          <TableCell className="font-bold text-on-surface">{studentName || "Desconhecido"}</TableCell>
                          <TableCell>{courseName || "Desconhecida"}</TableCell>
